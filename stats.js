...
function _buildTimelineDatasets() {
  ...
  return TIMER_IDS.map((id) => {
    ...
    segments.forEach((seg) => {
      const endSec = Math.round((seg.startedAt + seg.duration * 1000 - t0) / 1000);
      cum += seg.duration;
...
function _renderTimeline() {
  ...
  // Scale chart width: at least 520px, 60px per history segment for readability
  const wrapper = document.getElementById('timeline-wrapper');
  if (wrapper) {
    const minW = Math.max(520, history.length * 60);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          padding:         10,
          callbacks: {
            title: (items) => `@ ${fmt(Math.round(items[0].parsed.x))}`,
...
function _renderRadar(stats) {
  ...
  const data   = stats.map((s) => parseFloat((s.total / 60).toFixed(2)));
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: (ctx) => `  ${fmt(Math.round(ctx.raw * 60))}`,
          },
        },
      },
      scales: {
